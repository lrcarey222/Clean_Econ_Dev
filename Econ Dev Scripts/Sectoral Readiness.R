...
msa_data<-read.csv('OneDrive - RMI/Documents - US Program/6_Projects/Clean Regional Economic Development/ACRE/Data/Clean-growth-project/raw/ClimateandEconomicJusticeTool/msa_data_complete_dataset_q3_2024.csv')
...
#State Capacity
sc_data<-read.csv("OneDrive - RMI/Documents - US Program/6_Projects/Clean Regional Economic Development/ACRE/Data/Raw Data/sc_data.csv")
...
feas_simple <-  cgt_county_2 %>%
  select(county_name,county,industry_desc,density,rca) %>%
  left_join(msa_data %>%
              right_join(county_gdp %>%
                           select(GeoName,fips),by=c("geo_name"="GeoName")) %>%
              filter(geo=="County"),by=c("county"="fips")) %>%
  #State Capacity
  left_join(sc_data %>%
             select(state,SC),by=c("State"="state")) %>%
  #Taxes
  left_join(pdit,by=c("State"="full")) %>%
  left_join(popdens %>%
              mutate(GEOID=as.numeric(GEOID)) %>%
              rename("popdens"="B01001_cal"),by=c("county"="GEOID")) %>%
  ...
  #H2 Hubs
  mutate(h2hub = ifelse(state_abbr %in% c("PA",
                                    "WV",
                                    "OH",
                                    "CA",
                                    "TX",
...
feas_simple<-read.csv("C:/Users/LCarey/OneDrive - RMI/Documents/GitHub/Clean_Econ_Dev/Data/county_vars.csv")
...
cgt_ev<-cgt_county %>%
  ...
  left_join(facilities %>% 
              ...
              select(county_2020_geoid,Estimated_Total_Facility_CAPEX),
  ...
  summarize(correlation = cor(density, Estimated_Total_Facility_CAPEX, use = "complete.obs")) %>%
...
feas_simple_ev <- feas_simple %>%
  ...
  ungroup() %>%
  left_join(facilities %>% 
              select(Segment,Technology,county_2020_geoid,Estimated_Total_Facility_CAPEX) %>%
              filter(Segment=="Manufacturing",
                                  Technology %in% c("Zero Emission Vehicles", "Fueling Equipment")),
            by=c("county"="county_2020_geoid")) %>%
  mutate(Estimated_Total_Facility_CAPEX=ifelse(is.na(Estimated_Total_Facility_CAPEX),0,Estimated_Total_Facility_CAPEX),
         Investment_Flag=ifelse(Estimated_Total_Facility_CAPEX > 0, 1, 0)) 


# Standardize columns from density to Estimated_Total_Facility_CAPEX.y
feas_simple_ev_model <- feas_simple_ev %>%
  mutate(across(`2024_Clean`:energy_com, ~ scale(., center = TRUE, scale = TRUE)))
...
feas_simple_ev_index<-feas_simple_ev %>%
  ...
  mutate(ev_supply_chain_feas = 
           172402*coalesce(density, 0) +
           1.17*coalesce(rca, 0) +
           1.37*coalesce(gdp,0)+
           1.28*coalesce(prof_science_tech_5yrgdp ,0)+
           1.55*coalesce(man_pay,0)+
           1.46*coalesce(incent_gdp,0)+
           2.275*coalesce(demshare,0)+
           2.9*coalesce(road, 0) +
           0.7*coalesce(1-SC)+
           1.72*coalesce(ind_price_10yr)+
           0.7*coalesce(1-cnbc_rank)
         ) %>%
  mutate(across(where(is.numeric), ~ (. - min(., na.rm = TRUE)) / (max(., na.rm = TRUE) - min(., na.rm = TRUE)))) %>%
  #select(state_name,ea,ea_name,county_name,density,ev_supply_chain_feas) %>%
  select(-county)%>%
  right_join(county_gdp %>% select(-X2022),by=c("geo_name"="GeoName")) %>%
  distinct()



write.csv(feas_simple_ev_index %>%
            #mutate(ev_clus=ifelse(Estimated_Total_Facility_CAPEX>0,1,0)) %>%
            select(State,GeoFips,fips,geo_name,density,ev_supply_chain_feas),'C:/Users/LCarey/OneDrive - RMI/Documents - US Program/6_Projects/Clean Regional Economic Development/ACRE/Data/Raw Data/feas_simple_ev.csv',row.names=FALSE)

#compare with facilities data
feas_simple_ev_bins <-feas_simple_ev %>%
  left_join(feas_simple_ev_index %>%
              select(fips,ev_supply_chain_feas),by=c("county"="fips")) %>%
  ...
  left_join(facilities %>% filter(Segment=="Manufacturing",
                                  Technology %in% c("Zero Emission Vehicles")),
            by=c("county"="county_2020_geoid")) %>%
  mutate(Estimated_Total_Facility_CAPEX.y=ifelse(is.na(Estimated_Total_Facility_CAPEX.y),0,Estimated_Total_Facility_CAPEX.y)) 

feas_density_bins <- feas_simple_ev_bins %>%
  group_by(density_bin) %>%
  summarize_at(vars(Estimated_Total_Facility_CAPEX.y),sum,na.rm=T)%>%
  mutate(share=Estimated_Total_Facility_CAPEX.y/sum(Estimated_Total_Facility_CAPEX.y,na.rm=T)*100)
...
ggplot(data=feas_density_bins, aes(x=density_bin, y=Estimated_Total_Facility_CAPEX.y)) +
  geom_col() +
  theme_minimal()
...
feas_simple_ev_state <- feas_simple_ev_index %>%
              select(State,gdp,ev_supply_chain_feas) %>%
  group_by(State) %>%
  # Handle cases where county_gdp might have NAs
  summarize(ev_supply_chain_feas_weighted = weighted.mean(ev_supply_chain_feas, gdp, na.rm = TRUE))
...
feas_simple_batt_model <- feas_simple_batt %>%
  select(-geo,-geo_name,-inv_description,-topinv_desc,-top_industries,-partisan,-state_abbr,-State,-Region,-Division,-State.y,-Segment,-Technology) %>%
  mutate(across(density:Estimated_Total_Facility_CAPEX, ~ scale(., center = TRUE, scale = TRUE)))
...
feas_simple_batt_index<-feas_simple_batt %>%
  ...
  mutate(batt_feas = 
           ...
           4*coalesce(rca, 0) +
           1.9*coalesce(gdp, 0) +
           1.33*coalesce(incent_gdp, 0) +
           0.2*coalesce(1 - PropertyValueUSD , 0) +
           1.5*coalesce(demshare,0)+
           1.95*coalesce(manufacturing_lq, 0) +
           0.63*coalesce(manufacturing_lq, 0) +
           2.7*coalesce(road, 0) +
           0.6*coalesce(1-SC, 0) +
           0.6*coalesce(1 - cnbc_rank, 0)
          )%>%
  mutate(across(batt_feas,
...
ggplot(feas_simple_batt_index %>%
         filter(batt_feas != 0,
                Estimated_Total_Facility_CAPEX != 0), aes(x = batt_feas, y = Estimated_Total_Facility_CAPEX)) +
  geom_point(alpha = 0.5) +
  labs(title = "Scatterplot of Estimated_Total_Facility_CAPEX by batt_feas",
       x = "batt_feas",
       y = "Estimated_Total_Facility_CAPEX") +
  geom_smooth(method="lm",se=F)+
  theme_minimal()

# Density plot
feas_simple_batt_index <- feas_simple_batt_index %>%
  filter(!is.na(batt_feas), !is.na(Estimated_Total_Facility_CAPEX))
ggplot(feas_simple_batt_index %>%
         filter(batt_feas != 0,
                Estimated_Total_Facility_CAPEX != 0), aes(x = batt_feas, y = Estimated_Total_Facility_CAPEX)) +
  geom_density2d() +
  labs(title = "Density Plot of Estimated_Total_Facility_CAPEX by batt_feas",
       x = "batt_feas",
       y = "Estimated_Total_Facility_CAPEX") +
  theme_minimal()
...
write.csv(feas_simple_batt_index %>%
            select(GeoFips,fips,density,
                   batt_feas),
          'OneDrive - RMI/Documents - US Program/6_Projects/Clean Regional Economic Development/ACRE/Data/Raw Data/feas_simple_batt.csv',row.names=FALSE)

#compare with facilities data
feas_simple_batt_bins <-feas_simple_batt %>%
  left_join(feas_simple_batt_index %>%
              select(fips,batt_feas),by=c("county"="fips")) %>%
  mutate(density_bin=ntile(density,n=10),
...
feas_density_bins <- feas_simple_batt_bins %>%
  group_by(density_bin) %>%
  summarize_at(vars(Estimated_Total_Facility_CAPEX),sum,na.rm=T)%>%
  mutate(share=Estimated_Total_Facility_CAPEX/sum(Estimated_Total_Facility_CAPEX,na.rm=T))
...
cgt_solarman<-cgt_county %>%
  ...
  summarize(correlation = cor(rca, Estimated_Total_Facility_CAPEX, use = "complete.obs")) %>%
  arrange(desc(correlation))
...
feas_simple_solar_model <- feas_simple_solar %>%
  ...
  mutate(across(rca:Estimated_Total_Facility_CAPEX, ~ scale(., center = TRUE, scale = TRUE)))
...
feas_simple_solar_index <- feas_simple_solar %>%
  ...
  mutate(solar_feas = 
           ...
           #1.7*coalesce(Estimated_Total_Facility_CAPEX.x, 0) +
...
feas_simple_h2_model <- feas_simple_h2 %>%
  ...
  mutate(across(density_weighted:Estimated_Total_Facility_CAPEX, ~ scale(., center = TRUE, scale = TRUE)))
...
model <- glm(Investment_Flag ~ density + 
               ...
               tech_gen+
               #eci+
               #Estimated_Total_Facility_CAPEX.x+
               demshare+
               demshare_state+
               #BATHPR+
               #ren_cagr_20_23+
...
feas_density_bins <- feas_simple_h2_bins %>%
  group_by(density_bin) %>%
  summarize_at(vars(Estimated_Total_Facility_CAPEX.y),sum,na.rm=T)%>%
  mutate(share=Estimated_Total_Facility_CAPEX.y/sum(Estimated_Total_Facility_CAPEX.y)*100)
...
feas_simple_solar <- feas_simple %>%
  ...
  left_join(facilities %>% filter(Segment=="Energy and Industry",
            ...
            by=c("county"="county_2020_geoid")) %>%
  mutate(Estimated_Total_Facility_CAPEX.y=ifelse(is.na(Estimated_Total_Facility_CAPEX.y),0,Estimated_Total_Facility_CAPEX.y),
         Investment_Flag=ifelse(Estimated_Total_Facility_CAPEX.y > 0, 1, 0))  %>%
  distinct(state_name,ea,ea_name,county_name,density,
           rca,Estimated_Total_Facility_CAPEX.x,util_index,
...
feas_simple_solar_model_ols <- feas_simple_solar %>%
  ...
  left_join(facilities %>% filter(Segment=="Energy and Industry",
                                  Technology %in% c("Solar")),
            by=c("county"="county_2020_geoid")) %>%
  mutate(Estimated_Total_Facility_CAPEX.y=ifelse(is.na(Estimated_Total_Facility_CAPEX),0,Estimated_Total_Facility_CAPEX)) %>%
  select(state_name,ea,ea_name,county_name,density,
         rca,Estimated_Total_Facility_CAPEX.x,
         PropertyValueUSD ,tech_gen,ren_cagr_20_23,util_index,
         clean_electricity_policy_index,price,cnbc_rank,Estimated_Total_Facility_CAPEX.y) %>%
  mutate(across(density:Estimated_Total_Facility_CAPEX.y, ~ scale(., center = TRUE, scale = TRUE)))

# Run a OLS regression model to predict the probability of investment
model_ols <- lm(Estimated_Total_Facility_CAPEX.y ~ density + 
                  #rca +
                  #Estimated_Total_Facility_CAPEX.x+
                  tech_gen +
                  #PropertyValueUSD  +
...
feas_simple_solar_index<-feas_simple_solar %>%
  ...
  mutate(solar_feas = 
           ...
           #0.02*coalesce(rca, 0) +
           #1.277*coalesce(Estimated_Total_Facility_CAPEX.x, 0) +
  ...
  mutate(across(where(is.numeric), ~ (. - min(., na.rm = TRUE)) / (max(., na.rm = TRUE) - min(., na.rm = TRUE)))) %>%
  select(state_name,ea,ea_name,county_name,density,solar_feas,rca,Estimated_Total_Facility_CAPEX.x,PropertyValueUSD ,tech_gen,ren_cagr_20_23,clean_electricity_policy_index,price,cnbc_rank) %>%
  right_join(cgt_county %>% select(county,county_name),by=c("county_name")) %>%
  distinct()
...
feas_density_bins <- feas_simple_solar_bins %>%
  group_by(density_bin) %>%
  summarize_at(vars(Estimated_Total_Facility_CAPEX.y),sum,na.rm=T)%>%
  mutate(share=Estimated_Total_Facility_CAPEX.y/sum(Estimated_Total_Facility_CAPEX.y,na.rm=T))
...
feas_simple_wind <- feas_simple %>%
  ...
  distinct(state_name,ea,ea_name,county_name,density,
           rca,Estimated_Total_Facility_CAPEX.x,util_index,
           PropertyValueUSD ,tech_gen,ren_cagr_20_23,
           clean_electricity_policy_index,price,cnbc_rank,Investment_Flag,Estimated_Total_Facility_CAPEX.y)
...
feas_simple_wind_index<-feas_simple_wind %>%
  ...
  mutate(wind_feas = 
           ...
           12.3*coalesce(rca, 0) +
           #0.277*coalesce(Estimated_Total_Facility_CAPEX.x, 0) +
  ...
  mutate(across(where(is.numeric), ~ (. - min(., na.rm = TRUE)) / (max(., na.rm = TRUE) - min(., na.rm = TRUE)))) %>%
  select(state_name,ea,ea_name,county_name,density,wind_feas,rca,Estimated_Total_Facility_CAPEX.x,PropertyValueUSD ,tech_gen,util_index,ren_cagr_20_23,clean_electricity_policy_index,price,cnbc_rank) %>%
  right_join(cgt_county %>% select(county,county_name),by=c("county_name")) %>%
  distinct()
